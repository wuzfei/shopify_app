...
Route::fallback(function (Request $request) {
    ...
    if (Context::$IS_EMBEDDED_APP &&  $request->query("embedded", false) === "1") {
        ...
        $rUrl = Config::get("shopify.redirect_url");
        if ($rUrl) {
...
Route::get('/api/auth/callback', function (Request $request) {
    Log::info("/api/auth/callback", $request->all());
...
Route::get('/api/products/count', function (Request $request) {
    Log::info("/api/products/count", $request->all());
    /** @var AuthSession */
...
Route::get('/api/products/create', function (Request $request) {
    Log::info("/api/products/create", $request->all());
    /** @var AuthSession */
    $session = $request->get('shopifySession'); // Provided by the shopify.auth middleware, guaranteed to be active
...
Route::post('/api/webhooks', function (Request $request) {
    Log::info("/api/webhooks", $request->all());
    try {
